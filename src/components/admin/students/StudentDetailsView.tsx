        ...
        <Tabs defaultValue="overview" className="w-full">
          ...
          <div className="px-8 py-10">
            ...
            <TabsContent value="courses" className="m-0 space-y-8">
              ...
              <div className="overflow-hidden rounded-[36px] border border-slate-200 bg-white shadow-xl shadow-slate-200/40">
                <div className="flex items-center justify-between border-b border-slate-100 px-8 py-4 bg-slate-50/50">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">{filteredEnrollments.length} Active Records</span>
                  </div>
                  {loadingEnrollments && (
                    <div className="animate-pulse text-[10px] font-black uppercase tracking-widest text-indigo-500">Updating...</div>
                  )}
                </div>

                {filteredEnrollments.length === 0 ? (
                  <div className="p-20 text-center space-y-4">
                    <div className="h-16 w-16 rounded-[24px] bg-slate-50 border border-slate-100 flex items-center justify-center mx-auto text-slate-200">
                      <BookOpenCheck className="h-8 w-8" />
                    </div>
                    <p className="font-black text-[11px] uppercase tracking-[0.3em] text-slate-300">No enrollment history found</p>
                  </div>
                ) : (
                  <div className="divide-y divide-slate-50">
                    {filteredEnrollments.map((enrollment) => (
                      <div key={enrollment.id} className="group flex flex-col md:flex-row md:items-center justify-between px-8 py-8 hover:bg-slate-50/50 transition-all">
                        <div className="flex items-center gap-6">
                          <div className="h-16 w-16 rounded-[24px] bg-white border border-slate-100 flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform">
                            <BookOpenCheck className="h-7 w-7 text-slate-300 group-hover:text-indigo-500 transition-colors" />
                          </div>
                          <div className="space-y-2">
                            <span className="text-xl font-black text-slate-900 group-hover:text-indigo-600 transition-colors">{enrollment.course?.name || 'Untitled Course'}</span>
                            <div className="flex flex-wrap items-center gap-4">
                               <div className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-slate-100 text-[10px] font-black uppercase tracking-widest text-slate-500">
                                  ...
                                  {enrollment.branch?.name || 'Central'}
                               </div>
                               <div className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-indigo-50 text-[10px] font-black uppercase tracking-widest text-indigo-600">
                                  <History className="h-3 w-3" />
                                  {enrollment.batch?.name || 'Standard Batch'}
                               </div>
                               <span className="text-[10px] font-bold text-slate-300 uppercase tracking-widest">Enrolled: {format(new Date(enrollment.createdAt), 'dd-MM-yyyy')}</span>
                            </div>
                          </div>
                        </div>

                        <div className="mt-6 md:mt-0 flex items-center gap-4">
                          <Badge variant="outline" className={cn("rounded-xl px-4 py-2 text-[10px] font-black uppercase tracking-widest", getStatusBadgeClass(String(enrollment.status)))}>
                            {enrollment.status}
                          </Badge>
                          <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                              <Button variant="outline" size="icon" className="h-12 w-12 rounded-2xl border-slate-200 bg-white text-slate-400 hover:text-indigo-600 hover:border-indigo-600 shadow-sm transition-all">
                                <MoreVertical className="h-5 w-5" />
                              </Button>
                            </DropdownMenuTrigger>
                            <DropdownMenuContent align="end" className="w-56 p-2 rounded-2xl border-slate-200 bg-white shadow-2xl">
                              <DropdownMenuItem className="rounded-xl px-4 py-3 font-bold text-slate-600 hover:bg-slate-50 cursor-pointer" onClick={() => handleEditEnrollment(enrollment)}>
                                Edit Enrollment Details
                              </DropdownMenuItem>
                              <DropdownMenuItem className="rounded-xl px-4 py-3 font-bold text-rose-600 hover:bg-rose-50 cursor-pointer" onClick={() => handleCancelEnrollment(enrollment)}>
                                <Ban className="h-4 w-4 mr-3" />
                                Cancel Access
                              </DropdownMenuItem>
                              <DropdownMenuItem className="rounded-xl px-4 py-3 font-bold text-rose-600 hover:bg-rose-50 cursor-pointer" onClick={() => handleDeleteEnrollment(enrollment)}>
