  ...
  return (
    ...
    >
      <div className="grid gap-1">
        ...
        {description && (
          <div className="break-words text-sm opacity-90">{description}</div>
        )}
      </div>
      {onClose && (
        <button
...
export function Toaster({ toasts = [], removeToast = () => {} }: { toasts?: ToastProps[]; removeToast?: (id: string) => void }) {
  return (
    <div className="pointer-events-none fixed right-0 top-0 z-[100] flex max-h-screen w-full flex-col gap-2 p-4 md:max-w-[420px]">
      {(toasts ?? []).map((toast) => (
