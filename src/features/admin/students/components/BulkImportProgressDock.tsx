    ...
    <div className="pointer-events-auto w-[min(100vw-2rem,22rem)] rounded-2xl border border-slate-200 bg-white p-4 shadow-xl shadow-slate-200/80">
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0 flex-1">
          <p className="text-[10px] font-black uppercase tracking-wider text-slate-400">Bulk import</p>
          <p className="truncate text-sm font-bold text-slate-900">
            {job.originalName || 'Students file'}
          </p>
        </div>
      ...
      <p className="mt-2 text-xs font-semibold text-slate-600">
        ...
        {job.status !== 'CANCELLED' && (
          <>
            {' · '}
            <span className="text-emerald-700">{job.createdCount} created</span>
            {job.errorCount > 0 && (
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      for (const job of list) {
        if (cancelled) return;
        try {
          const res = await getBulkImportJobStatus(job.jobId);
          ...
          if (finished && !toastedFinished.current.has(job.jobId)) {
            ...
            if (typeof window !== 'undefined') {
              window.dispatchEvent(
                new CustomEvent(BULK_STUDENT_IMPORT_COMPLETE_EVENT, {
                  detail: {
                    jobId: job.jobId,
                    createdCount: d.createdCount,
                    errorCount: d.errorCount,
            ...
            if (d.status === 'COMPLETED') {
              toast({
                ...
                description: `Created ${d.createdCount} student(s). ${d.errorCount > 0 ? `${d.errorCount} row(s) skipped.` : ''}`,
