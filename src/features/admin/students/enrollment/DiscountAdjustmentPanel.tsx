  ...
  return (
    <div>
      ...
      <div className="flex justify-between items-center">
        <Button variant="outline" onClick={onBack} className="gap-2" disabled={isApplying}>
          <ArrowLeft className="h-4 w-4" /> Go Back
        </Button>
        <Button
          onClick={() => !discountExceedsTotal && onApply(appliedDiscount, distributed)}
          disabled={discountExceedsTotal || isApplying}
          className="gap-2 bg-emerald-600 text-white hover:bg-emerald-700 transition-all"
        >
          <Check className="h-4 w-4" /> {isApplying ? (applyLabel ?? 'Applying...') : 'Apply & Generate Updated Invoices'}
        </Button>
      </div>
    </div>
  );
