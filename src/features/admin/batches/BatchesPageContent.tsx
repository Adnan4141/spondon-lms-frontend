...
export function BatchesPageContent() {
  ...
  const {
    ...
    error: queryError,
    refetch,
  } = useBatchesList(batchQueryParams, branchScopeReady);
    ...
    try {
      ...
      if (response.success && response.data) {
        openModal({
          ...
          content: <BatchForm courses={courses} branches={branches} batch={response.data} onSuccess={invalidateBatches} />,
        });
      }
    } catch {
  ...
  const handleCreateBatch = () => {
    openModal({
      ...
      className: 'w-[calc(100vw-1rem)] sm:max-w-2xl',
      content: <BatchForm courses={courses} branches={scopedBranchId ? branches.filter((b) => b.id === scopedBranchId) : branches} onSuccess={invalidateBatches} />,
    });
  };
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div className="grid flex-1 gap-3 sm:grid-cols-2 lg:grid-cols-[minmax(260px,1fr)_180px_180px_180px_48px] lg:items-center">
            ...
            <Button variant="outline" className="h-12 w-full rounded-2xl border-slate-200 bg-white p-0 text-slate-400 hover:bg-slate-50 hover:text-indigo-600 transition-all shadow-sm sm:w-12" onClick={() => void refetch()}>
