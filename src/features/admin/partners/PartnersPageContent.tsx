...
export function PartnersPageContent() {
  ...
  const { data: partners = [], isLoading: loading, isFetching, refetch } = usePartnersList();
  ...
  const openPartnerDetails = (p: PartnerAdmin) => {
    setDetailPartner(p);
            ...
            <div className="space-y-6 py-1">
              <div className="flex gap-4">
                <div className="flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-2xl border border-slate-100 bg-white p-2">
                  {displayPartner.logo ? (
                    <img
                      src={`${API_ORIGIN}${displayPartner.logo}`}
                      alt=""
                      className="max-h-full max-w-full object-contain"
                    />
                  ) : (
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-lg font-black text-slate-900">{displayPartner.name}</p>
                  <Badge variant="outline" className="mt-1 rounded-lg text-[9px] font-black uppercase">
                    {displayPartner.type || 'Partner'}
                  </Badge>
                  {displayPartner.websiteUrl ? (
                    <a
                      href={displayPartner.websiteUrl}
                      target="_blank"
                      rel="noreferrer"
                      className="mt-2 inline-flex items-center gap-1 text-xs font-bold text-indigo-600 hover:underline"
                    >
                      {safeHostname(displayPartner.websiteUrl)}
                      <ExternalLink className="h-3 w-3" />
                    </a>
                  ) : null}
                  <p className="mt-2 text-[10px] font-bold uppercase tracking-widest text-slate-400">
                    Sort {displayPartner.sortOrder} · Homepage {displayPartner.isActive ? 'live' : 'hidden'}
                    {displayPartner.revenueSharePercent != null
                      ? ` · Rev share ${Number(displayPartner.revenueSharePercent)}%`
                      : ''}
                  </p>
                </div>
              </div>
              {displayPartner.description ? (
                <p className="text-sm leading-relaxed text-slate-600">{displayPartner.description}</p>
              ) : null}
              ...
              <div className="space-y-4 border-t border-slate-100 pt-4">
                ...
                {(displayPartner.partnerCourses?.length ?? 0) === 0 ? (
                  <p className="text-xs text-slate-400">None</p>
                ) : (
                  <ul className="space-y-2">
                    {displayPartner.partnerCourses!.map((row) => (
              ...
              <div className="space-y-4 border-t border-slate-100 pt-4">
                ...
                {(displayPartner.partnerBooks?.length ?? 0) === 0 ? (
                  <p className="text-xs text-slate-400">None</p>
                ) : (
                  <ul className="space-y-2">
                    {displayPartner.partnerBooks!.map((row) => (
                      <li key={row.book.id}>
                        <Button
                          type="button"
                          variant="outline"
