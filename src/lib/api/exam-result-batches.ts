...
export async function getExamResultBatchDetail(examId: string, batchId: string): Promise<ApiResponse<any>> {
  return apiRequest<ApiResponse<any>>(`/exams/${examId}/results/batches/${batchId}`);
}
...
export async function postExamResultBulkExcel(examId: string, file: File, branchId?: string): Promise<ApiResponse<any>> {
  const uid = getActorUserIdFromStorage();
  if (!uid) throw new Error('Sign in required');
...
export async function approveResultBatchBranch(examId: string, batchId: string): Promise<ApiResponse<any>> {
  const uid = getActorUserIdFromStorage();
  if (!uid) throw new Error('Sign in required');
...
export async function deleteResultBatch(examId: string, batchId: string): Promise<ApiResponse<any>> {
