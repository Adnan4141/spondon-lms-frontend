...
function RegisterForm() {
  ...
  // Read query params on mount: ?mobile=xxx&step=otp allows deep-linking directly into OTP step
  useEffect(() => {
    const mobileParam = searchParams.get('mobile');
    const stepParam = searchParams.get('step');
    if (stepParam === 'otp' && mobileParam) {
      setRegisteredMobile(mobileParam);
      setStep('otp');
      // Auto-trigger resend so a fresh OTP is sent (user may have come from login banner)
      resendOtp({ mobile: mobileParam, purpose: 'REGISTRATION' }).catch(() => {});
      startResendCooldown();
    }
    // Only run on mount
        ...
        <div className="relative z-10 w-full max-w-md space-y-12">
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4 pt-4">
              {['সেরা মেন্টরদের তত্ত্বাবধানে পূর্ণাঙ্গ প্রস্তুতি', 'আধুনিক প্রযুক্তিনির্ভর লার্নিং ম্যানেজমেন্ট', 'সারা দেশে বিস্তৃত ব্রাঞ্চ নেটওয়ার্ক'].map((text, i) => (
            ...
            {step === 'form' ? (
              <motion.div key="form" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-10">
                <header className="space-y-4">
                  ...
                  <p className="text-slate-500 font-bold text-lg">নিচের তথ্যগুলো দিয়ে আপনার একাউন্টটি সুরক্ষিত করুন।</p>
                </header>
                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                  <div className="grid gap-6 sm:grid-cols-2">
                    ...
                    <div className="space-y-2 sm:col-span-2">
                      ...
                      <div className="relative group">
                        ...
                        <Input type="text" placeholder="আপনার পুরো নাম" value={formData.fullName} onChange={(e) => setFormData(p => ({ ...p, fullName: e.target.value }))} className={inputStyles} />
                      </div>
                    </div>
                    ...
                    <div className="space-y-2">
                      <label className={labelStyles}>পাসওয়ার্ড</label>
                      <div className="relative group">
                      ...
                      {formData.password.length > 0 && (
                        <p className={cn('text-[13px] font-black uppercase tracking-wider ml-1 mt-1 flex items-center gap-1', formData.password.length < 6 ? 'text-rose-500' : 'text-emerald-500')}>
                          {formData.password.length < 6 ? <><span className="h-1 w-1 rounded-full bg-rose-500" /> পাসওয়ার্ড অন্তত ৬ অক্ষরের হতে হবে</> : <><span className="h-1 w-1 rounded-full bg-emerald-500" /> পাসওয়ার্ডের দৈর্ঘ্য সঠিক আছে</>}
                        </p>
                      )}
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      {formData.confirmPassword.length > 0 && (
                        <p className={cn('text-[13px] font-black uppercase tracking-wider ml-1 mt-1 flex items-center gap-1', formData.password !== formData.confirmPassword ? 'text-rose-500' : 'text-emerald-500')}>
                          {formData.password !== formData.confirmPassword ? <><span className="h-1 w-1 rounded-full bg-rose-500" /> পাসওয়ার্ড মেলেনি</> : <><span className="h-1 w-1 rounded-full bg-emerald-500" /> পাসওয়ার্ড মিলেছে</>}
                        </p>
                      )}
                    </div>
                  </div>
                  ...
                  >
                    {isLoading ? <div className="h-5 w-5 border-[3px] border-white/30 border-t-white rounded-full animate-spin" /> : <>একাউন্ট তৈরি করুন <ArrowRight className="h-5 w-5" /></>}
                  </Button>
                </form>
                ...
                <header className="space-y-4">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="h-14 w-14 rounded-2xl bg-indigo-50 border-2 border-indigo-100 flex items-center justify-center">
                      <MessageSquare className="h-7 w-7 text-[#5C2D91]" />
                    </div>
                    <div>
                      <h1 className="text-3xl font-black text-slate-900 tracking-tighter">মোবাইল যাচাই করুন</h1>
                      <p className="text-slate-500 font-bold text-sm mt-1">
                        <span className="text-[#5C2D91] font-black">{registeredMobile}</span> নম্বরে কোড পাঠানো হয়েছে
                      </p>
                    </div>
                  </div>
                </header>
