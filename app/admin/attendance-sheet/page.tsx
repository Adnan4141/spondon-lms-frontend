...
function AttendanceSheetInner() {
  ...
  // ── load courses on program change ────────────────────────────────────────
  useEffect(() => {
    if (!selectedProgramId) { setCourses([]); setSelectedCourseId(''); return; }
    setLoadingCourses(true);
    getCourses({ programId: selectedProgramId })
      .then((r) => setCourses(r.data ?? []))
      .catch(() => {})
      .finally(() => setLoadingCourses(false));
  }, [selectedProgramId]);

  // ── load branches on course change ────────────────────────────────────────
  useEffect(() => {
    if (!selectedCourseId) { setBranches([]); setSelectedBranchId(''); return; }
