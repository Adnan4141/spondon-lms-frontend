  ...
  return (
    <div className="space-y-8 text-slate-900">
      ...
      <section className="grid gap-6 sm:grid-cols-2 xl:grid-cols-4">
        {[
          { label: 'Mapped Assets', value: courseBooks.length, color: 'from-blue-600 to-cyan-500', icon: Link },
          { label: 'Complementary', value: courseBooks.filter(cb => cb.isFree).length, color: 'from-emerald-600 to-teal-500', icon: CheckCircle2 },
          { label: 'Premium Addons', value: courseBooks.filter(cb => !cb.isFree).length, color: 'from-amber-600 to-orange-500', icon: ShoppingBag },
          { label: 'Context Status', value: selectedCourseId ? 'Mapped' : 'Awaiting', color: 'from-purple-600 to-indigo-600', icon: Layers },
        ].map((stat, i) => (
