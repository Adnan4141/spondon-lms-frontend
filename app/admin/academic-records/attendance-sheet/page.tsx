  ...
  const handleGenerateSheet = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        toast({ title: 'Done', description: 'Attendance sheet generated.', variant: 'success' });
      }
    } catch (err: any) {
    ...
    <div className="space-y-8 text-slate-900 pb-20">
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 px-2 print:hidden">
        <div className="space-y-1">
          <h1 className="text-3xl font-black tracking-tight">Attendance Sheet</h1>
          <p className="text-slate-500 font-medium flex items-center gap-2">
            <Users className="h-4 w-4 text-indigo-500" />
            Track attendance by course, branch and batch
          </p>
        </div>
        {sheetData && (
      ...
      {sheetData ? (
        <section className="rounded-[40px] border border-slate-200 bg-white overflow-hidden shadow-2xl shadow-slate-200/50 animate-in fade-in slide-in-from-bottom-4 duration-500 print:shadow-none print:border-none print:rounded-none">
          ...
          <div className="hidden print:block p-10 border-b-2 border-slate-900 mb-8">
             <div className="flex justify-between items-start">
                <div>
                   <h1 className="text-4xl font-black uppercase tracking-tighter text-slate-900">Attendance Sheet</h1>
                   <div className="mt-4 space-y-1">
                      <p className="text-lg font-bold text-slate-700">COURSE: {sheetData.course.name} ({sheetData.course.code})</p>
                      <p className="text-sm font-medium text-slate-500">BRANCH: {filters.branchId !== 'all' ? branches.find(b => b.id === filters.branchId)?.name : 'All Branches'}</p>
                ...
                <div className="text-right">
                   <p className="text-xs font-black uppercase tracking-widest text-slate-400">Generated</p>
          ...
          <div className="px-10 py-8 border-b border-slate-100 flex items-center justify-between bg-slate-50/30 print:hidden">
             <div className="flex items-center gap-4">
                ...
                <div>
                   <h3 className="text-xl font-black text-slate-900 tracking-tight">Attendance</h3>
                   <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">{sheetData.enrollments.length} Students • {sheetData.sessions.length} Sessions</p>
                </div>
             </div>
          </div>
          ...
          <div className="overflow-x-auto print:overflow-visible">
            <table className="w-full text-left border-collapse">
              <thead className="bg-slate-50 border-b border-slate-200 sticky top-0 z-10 print:static">
                <tr>
                  <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 border-r border-slate-100 sticky left-0 bg-slate-50 z-20">Student</th>
                  ...
                  {sheetData.sessions.length === 0 && (
                    <th className="px-8 py-10 text-center text-[10px] font-black uppercase tracking-[0.3em] text-slate-300">No sessions found</th>
                  )}
                </tr>
              </thead>
        ...
        <section className="flex flex-col items-center justify-center p-20 rounded-[40px] border-2 border-dashed border-slate-200 bg-slate-50/50 space-y-6">
           ...
           <div className="text-center space-y-2">
              <h3 className="text-xl font-black text-slate-400 tracking-tight">No Data</h3>
              <p className="text-sm font-medium text-slate-400 max-w-md mx-auto">Select a course and branch to generate the attendance sheet.</p>
           </div>
