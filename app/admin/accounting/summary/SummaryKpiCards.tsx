...
export function SummaryKpiCards({ summary }: Props) {
  const branchLinkedBalance = summary.branchLinked?.balance ?? 0;
  const items = [
    ...
    {
      label: 'Branch-linked Net',
      value: branchLinkedBalance,
      icon: Building2,
      color: branchLinkedBalance >= 0 ? 'text-sky-700' : 'text-rose-600',
      bg: 'bg-sky-50',
    },
    ...
    <div className="grid grid-cols-2 gap-4 xl:grid-cols-4 2xl:grid-cols-7">
      {items.map((kpi) => (
        <div key={kpi.label} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          {kpi.label === 'Branch-linked Net' && summary.branchLinked ? (
            <p className="mt-1 text-[10px] font-semibold text-slate-500">
              {summary.branchLinked.entryCount} HO entries tagged with a branch
            </p>
          ) : null}
        </div>
      ))}
