        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            ...
            >
                <div className="px-6 pb-6 pt-1 border-t border-slate-100">
                    <p className="text-slate-600 leading-relaxed font-medium text-sm ml-12">{item.answer}</p>
                </div>
            </div>
        </div>
    );
    ...
    useEffect(() => {
        getSiteSettings().then((res) => {
            if (res.success && res.data) {
                ...
                setSiteSettings(map);
                try {
                    setItems(JSON.parse(map['pages.faq_items'] ?? '[]'));
                } catch {
                    setItems([]);
                }
            }
            ...
            <div className="max-w-3xl mx-auto px-6 lg:px-12 py-16">
                {loading ? (
                    ...
                    <div className="bg-white rounded-3xl border border-dashed border-slate-200 p-16 text-center">
                        ...
                        <p className="text-slate-400 font-bold text-lg">এখনো কোনো প্রশ্ন-উত্তর যোগ করা হয়নি।</p>
