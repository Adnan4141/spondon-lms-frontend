...
export default function StudentRoutinePage() {
  const mockSchedule = [
    { id: 1, subject: 'Advanced Mathematics', time: '09:00 AM - 10:30 AM', teacher: 'Dr. Sarah Wilson', room: 'Room 302', type: 'Lecture' },
    { id: 2, subject: 'Physics Lab', time: '11:00 AM - 12:30 PM', teacher: 'Prof. James Bond', room: 'Lab 101', type: 'Practical' },
    { id: 3, subject: 'English Literature', time: '02:00 PM - 03:30 PM', teacher: 'Ms. Emily Blunt', room: 'Online', type: 'Webinar' },
  ];
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        <div className="flex items-center gap-3">
           <div className="px-5 py-3 rounded-2xl bg-white border border-slate-100 shadow-sm flex items-center gap-3">
              ...
              <span className="text-sm font-black text-slate-900">Tuesday, March 17</span>
           </div>
        </div>
      </div>
            ...
            <div className="flex items-center justify-between px-2">
               ...
               <div className="flex gap-2">
                  {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((day) => (
                     <button key={day} className={`px-4 py-2 rounded-xl text-xs font-black transition-all ${day === 'Tue' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-100' : 'bg-white text-slate-400 hover:bg-slate-50'}`}>
                        {day}
                     </button>
                  ))}
               </div>
            ...
            {mockSchedule.length === 0 ? (
               ...
               <div className="space-y-4">
                  {mockSchedule.map((item) => (
                     <Card key={item.id} className="group overflow-hidden rounded-[2rem] border-none bg-white shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_20px_40px_rgba(0,0,0,0.08)] transition-all duration-500">
                              ...
                              <div className="flex-1 p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                                 <div className="flex items-center gap-6">
                                    ...
                                    <div className="space-y-1">
                                       ...
                                       <h3 className="text-xl font-black text-slate-900 group-hover:text-indigo-600 transition-colors">{item.subject}</h3>
                                    </div>
                                 </div>
                                 <div className="flex flex-col sm:items-end gap-1">
                                    ...
                                    <p className="text-sm text-slate-400 font-medium">with {item.teacher}</p>
                                 </div>
